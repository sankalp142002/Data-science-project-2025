...
def plot_horizon_from_samples(dsamp: pd.DataFrame, span: str | None):
    ...
    for m, grp in agg.groupby("model"):
        # add small markers to each line for clarity
        ax.plot(
            grp.h, clip_pos(grp.rmse_gmean), 
            marker="o", markersize=3,   # smaller points
            lw=2 if m == args.baseline else 1.5, 
            label=m
        )
...
G_SEMI = make_out(RES2, "semi_major_axis_grids")
G_CURV = make_out(RES2, "sgp4_curves")

def _prepare_sma_curves(dlog: pd.DataFrame) -> pd.DataFrame:
    """
    Tidy frame with (model, span, w, h, RMSE) for semi_major_axis,
    using median rmse_log per cell → RMSE. Robust to missing 'span'/'w'.
    """
    need = {"model","feature","h","rmse_log"}
    if not need.issubset(dlog.columns): return pd.DataFrame()

    sub = dlog[(dlog.feature == "semi_major_axis") & dlog["h"].notna()].copy()

    group_keys = ["model"]
    if "span" in sub.columns: group_keys.append("span")
    if "w" in sub.columns:    group_keys.append("w")
    group_keys.append("h")

    agg = (sub.groupby(group_keys, dropna=False)["rmse_log"].median().reset_index())
    agg["RMSE"] = 10.0 ** agg["rmse_log"]

    # Provide defaults if columns are missing
    if "span" not in agg.columns: agg["span"] = "full"
    if "w"    not in agg.columns: agg["w"]    = -1
    return agg

def _nice_log_ylim(values, pad_low=0.6, pad_high=1.8, floor=1e-4, ceil=1e3):
    v = np.asarray(values, float)
    v = v[np.isfinite(v) & (v > 0)]
    if v.size == 0: return (1e-2, 2e1)
    lo, hi = v.min(), v.max()
    lo = max(lo*pad_low, floor); hi = min(hi*pad_high, ceil)
    if hi <= lo: hi = lo * 10.0
    return (lo, hi)

def plot_sma_grid_all_models(dlog: pd.DataFrame,
                             span_order: list[str] | None = None,
                             baseline: str = "sgp4_monotonic"):
    """
    Grid of RMSE(h) for semi_major_axis across ALL models, with the
    baseline (sgp4_monotonic) **explicitly overlaid in every panel**.
    Rows = spans, Cols = window sizes w.
    """
    D = _prepare_sma_curves(dlog)
    if D.empty:
        print("ℹ️  SMA grid skipped: required columns not found in dlog.")
        return

    # Determine panel orders
    if span_order is None:
        pref = ["last90d","last180d","last365d","last730d","full"]
        span_order = [s for s in pref if s in set(D.span)]
        if not span_order:
            span_order = sorted(D.span.unique(), key=lambda x: (x!="full", x))
    w_order = [w for w in [30, 60, 90] if w in set(D.w)]
    if not w_order: w_order = sorted(D.w.unique())
    nrows, ncols = len(span_order), len(w_order)
    if nrows == 0 or ncols == 0:
        print("ℹ️  SMA grid skipped: no spans or windows to plot.")
        return

    # Build a span→(h, RMSE) map for the baseline, ignoring w
    has_baseline = baseline in set(D.model)
    baseline_map = {}
    if has_baseline:
        base_span_h = (D[D.model == baseline]
                         .groupby(["span","h"], dropna=False)["RMSE"]
                         .median().reset_index())
        # ensure all requested spans exist (fallback to overall if missing)
        all_h = sorted(D["h"].unique())
        for sp in span_order:
            b = base_span_h[base_span_h["span"] == sp]
            if b.empty:
                # fallback: overall median across spans
                b = (D[D.model == baseline].groupby("h")["RMSE"]
                        .median().reindex(all_h).reset_index().rename(columns={"RMSE":"RMSE"}))
                b["span"] = sp
            else:
                b = b.sort_values("h")
            baseline_map[sp] = b[["h","RMSE"]].copy()

    # Shared y-range across all panels (include baseline values)
    yvals = D["RMSE"].values
    if has_baseline:
        yvals = np.concatenate([yvals] + [v["RMSE"].values for v in baseline_map.values()])
    ylo, yhi = _nice_log_ylim(yvals)
    ...
    for i, sp in enumerate(span_order):
        for j, w in enumerate(w_order):
            ax = axes[i, j]
            panel = D[(D.span == sp) & (D.w == w)]

            # Plot non-baseline models first
            for m, g in panel[panel.model != baseline].groupby("model", sort=False):
                g = g.sort_values("h")
                (ln,) = ax.plot(
                    g.h, g.RMSE,
                    marker="o", markersize=3.5,
                    lw=1.4, alpha=0.95, label=m
                )
                if m not in model_lines: model_lines[m] = ln

            # Overlay baseline in every panel (broadcast over w)
            if has_baseline:
                b = baseline_map[sp]
                (lnb,) = ax.plot(
                    b.h, b.RMSE,
                    marker="o", markersize=3.5,
                    lw=2.2, alpha=0.95, label=baseline
                )
                if baseline not in model_lines: model_lines[baseline] = lnb

            # Cosmetics
            ax.set_yscale("log"); ax.set_ylim(ylo, yhi)
            ax.grid(True, which="both", alpha=0.25, linewidth=0.6)
            if i == 0:         ax.set_title(f"w = {w}", fontsize=11)
            if j == 0:         ax.set_ylabel(f"{sp}\nRMSE", fontsize=10)
            if i == nrows - 1: ax.set_xlabel("horizon (d)", fontsize=10)

    handles = [model_lines[m] for m in model_lines]
    labels  = list(model_lines.keys())
    fig.legend(handles, labels, ncol=min(len(labels), 6),
               loc="upper center", bbox_to_anchor=(0.5, 1.02),
               fontsize=9, frameon=False, handlelength=2.2, columnspacing=1.2)
    fig.suptitle("RMSE of semi_major_axis across models", y=1.08, fontsize=13)
    fig.tight_layout()
    savefig_loud(fig, G_SEMI / "sma_grid_all_models.png")

def plot_sgp4_simple_curve(dlog: pd.DataFrame, baseline: str = "sgp4_monotonic"):
    ...
    ylo, yhi = _nice_log_ylim(agg["RMSE"].values)

    fig, ax = plt.subplots(figsize=(6.6, 4.2))
    ax.plot(agg.h, agg.RMSE, marker="o", markersize=3.8, lw=2.2, label=baseline)
    ax.set_yscale("log"); ax.set_ylim(ylo, yhi)
    ax.set_xlabel("horizon (d)"); ax.set_ylabel("RMSE")
    ax.set_title("SGP-4 monotonic – semi_major_axis")
    ax.grid(True, which="both", alpha=0.3, linewidth=0.6)
    ax.legend(frameon=False)
    fig.tight_layout()
    savefig_loud(fig, G_CURV / "sgp4_semimajoraxis_curve.png")
