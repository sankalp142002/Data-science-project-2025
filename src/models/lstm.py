...
Usage
-----
python src/models/lstm_baseline.py \
...
class LSTMForecast(pl.LightningModule):
    ...
    def _step(self, batch, split:str):
        ...
        for b in ["inclination","raan","arg_perigee","mean_anomaly"]:
            try:
                s_idx = self.feats.index(f"{b}_sin")
                c_idx = self.feats.index(f"{b}_cos")
            except ValueError:
                continue
            a_loss += wrapped_rad(out[:,s_idx], out[:,c_idx],
                                  y[:,s_idx],   y[:,c_idx])
        loss = mse + a_loss
 ...
 # local import avoids cycles
    ...
    for i,name in enumerate(feats):
        ...
        if name in {"eccentricity","semi_major_axis","mean_motion"}:
            ...
            mape = np.mean(np.abs(P_inv[:,i]-T_inv[:,i])/(np.abs(denom)+1e-8))*100
    ...
    for base in ("inclination", "raan", "arg_perigee", "mean_anomaly"):
        ...
        # (optional) include angle into overall accuracy numerator
        # mape_core.append(rmse_deg / 180 * 100)  # ≤ add if desired
    overall = 100 - float(np.mean(mape_core)) if mape_core else None

    out = dict(metrics=native(metrics),
               overall_acc_percent=overall,
               hidden=hidden, layers=layers,
               epochs=epochs, batches=batch)
    out_dir = Path("results") / "lstm"
